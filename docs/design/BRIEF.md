...
- No analytics, no cookies, and no requests to anyone but this site from a visit. The site's own scripts are the theme toggle, the front page's dateline clock, and the front page's weather refresh: three small first-party scripts and nothing from anyone else. Weather is read when the site is built **and** refreshed on load from `wx.kj5irq.radio/weather.json` — our own Worker on our own subdomain, caching the National Weather Service for five minutes. Same registrable domain, so no third party is involved and doc 14's P8 keeps passing; the Worker allows the read from `https://kj5irq.radio` and nothing else. It is a subdomain rather than a path on the apex because a Worker route only runs on a Cloudflare-proxied hostname and the apex is a DNS-only CNAME to GitHub Pages.
- The site depends on exactly one runtime: that Worker. If it is removed, the page falls back to the build-time reading with no site change, which is deliberate.
